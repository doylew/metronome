...
public:
    ...
    class State {
    public:
        bool operator==(const State& state) const { return state.zeroIndex == zeroIndex && state.tiles == tiles; }

        bool operator!=(const State& state) const { return state.tiles != tiles; }

        std::size_t hash() const {
            return (size_t)tiles >> 32 ^ tiles;
        }
        ...
        void set(const unsigned char index, const unsigned char value) {
            tiles = (tiles & ~(15ULL << (index * 4))) | static_cast<unsigned long long int>(value) << (index * 4);
        }
        ...
        friend std::ostream& operator<<(std::ostream& os, const State& state) {
            os << "zero: " << static_cast<int>(state.zeroIndex) << " tiles: " << state.tiles;
            return os;
        }
    ...
    SlidingTilePuzzle(const Configuration& configuration, std::istream& input)
        ...
        if (line.empty()) {
            throw MetronomeException("First line of the domain description can't be empty. Path might be invalid.");
        }
    ...
    std::vector<SuccessorBundle<SlidingTilePuzzle>> successors(const State& state) const {
...
private:
    ...
    void addValidSuccessor(std::vector<SuccessorBundle<SlidingTilePuzzle>>& successors,
    ...
    boost::optional<State> getSuccessor(const State& sourceState, signed char relativeX, signed char relativeY) const {
        signed char targetZeroIndex =
                static_cast<signed char>(sourceState.getZeroIndex()) + getIndex(relativeX, relativeY);

        if (targetZeroIndex >= 0 && targetZeroIndex < size * size) {
            State targetState{sourceState};

            // Move tile
            targetState.set(targetState.getZeroIndex(), targetState[static_cast<unsigned char>(targetZeroIndex)]);
